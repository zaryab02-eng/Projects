...
export function Footer() {
  ...
  return (
    <footer className="border-t border-white/5 bg-charcoal">
      <div className="container-px py-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        <div>
          <p className="font-display text-2xl text-ivory mb-3">
            {siteConfig.shopName}
          </p>
          <p className="body-copy text-sm max-w-xs">
            {siteConfig.tagline} — a family workshop trusted across four
            generations.
          </p>
          <div className="flex gap-4 mt-6">
            <a
              ...
              className="text-ash hover:text-brass-light transition-colors"
            >
              <Instagram size={18} />
            </a>
            <a
          ...
          <ul className="space-y-3">
            {quickLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="body-copy text-sm hover:text-brass-light transition-colors"
                >
                  {link.label}
                </a>
              </li>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {siteConfig.contact.hours.map((slot) => (
              <li
                key={slot.day}
                className="flex justify-between gap-4 body-copy"
              >
                <span>{slot.day}</span>
