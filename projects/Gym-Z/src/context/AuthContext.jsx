...
import { getGym } from '../firebase/firestore.js'
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        try {
          const gymDoc = await getGym(firebaseUser.uid)
          setGym(gymDoc)
        } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{ user, gym, gymId: user?.uid || null, loading, refreshGym, setGym }}>
      {children}
