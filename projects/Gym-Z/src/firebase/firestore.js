...
export async function listRankedGyms() {
  const q = query(collection(db, 'gyms'), orderBy('activeMemberCount', 'desc'), limit(100))
...
export async function addPlan(gymId, plan) {
  return addDoc(plansCollection(gymId), { ...plan, createdAt: serverTimestamp() })
}
...
export function subscribeToPlans(gymId, callback) {
  return onSnapshot(query(plansCollection(gymId), orderBy('durationDays', 'asc')), (snap) => {
...
export async function findMemberByPhone(gymId, phone) {
  const q = query(membersCollection(gymId), where('phone', '==', phone), limit(1))
  const snap = await getDocs(q)
...
export async function renewMembership(gymId, memberId, renewalData) {
  ...
  const renewalRef = doc(collection(db, 'gyms', gymId, 'members', memberId, 'renewals'))
...
export function subscribeToMembers(gymId, callback) {
  return onSnapshot(query(membersCollection(gymId), orderBy('expiryDate', 'asc')), (snap) => {
    callback(snap.docs.map((d) => ({ id: d.id, ...d.data() })))
  })
}
...
export async function getMemberRenewals(gymId, memberId) {
  const snap = await getDocs(
    query(collection(db, 'gyms', gymId, 'members', memberId, 'renewals'), orderBy('createdAt', 'desc'))
  )
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }))
}
...
export async function addToBlacklist(gymId, memberId, entry) {
  ...
  await updateDoc(doc(db, 'gyms', gymId, 'members', memberId), { blacklisted: true })
}
...
export async function removeFromBlacklist(gymId, blacklistEntryId, memberId) {
  ...
  await updateDoc(doc(db, 'gyms', gymId, 'members', memberId), { blacklisted: false })
}

export function subscribeToBlacklist(gymId, callback) {
  return onSnapshot(query(blacklistCollection(gymId), orderBy('dateAdded', 'desc')), (snap) => {
    callback(snap.docs.map((d) => ({ id: d.id, ...d.data() })))
  })
}
