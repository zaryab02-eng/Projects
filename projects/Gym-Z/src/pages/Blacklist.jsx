...
export default function Blacklist() {
  ...
  useEffect(() => {
    if (gymId) return subscribeToBlacklist(gymId, setEntries);
  }, [gymId]);
  useEffect(() => {
    if (gymId) return subscribeToMembers(gymId, setMembers);
  }, [gymId]);
  ...
  const handleRemove = async (entry) => {
    if (confirm("Remove this member from the blacklist?")) {
      await removeFromBlacklist(gymId, entry.memberId);
    }
  };
    ...
    <AppShell>
      ...
      {!entries ? (
        ...
        <Card className="p-10 text-center text-ink-500 text-sm">
          No blacklisted members. Blacklist entries are added from a member's
          profile page.
        </Card>
      ) : (
