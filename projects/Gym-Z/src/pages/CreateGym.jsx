...
export default function CreateGym() {
  ...
  const handleCreateGym = async (e) => {
    ...
    try {
      ...
      const loginEmail = form.email.trim() || `${phone.replace(/[^0-9]/g, '')}@gymz.app`
        ...
        <Card className="w-full max-w-md p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-6">
            {[STEPS.PHONE, STEPS.OTP, STEPS.DETAILS].map((s, i) => (
              <div key={s} className={`h-1.5 flex-1 rounded-full ${
                [STEPS.PHONE, STEPS.OTP, STEPS.DETAILS].indexOf(step) >= i ? 'bg-copper-500' : 'bg-ink-700'
              }`} />
            ))}
          </div>
          ...
          {step === STEPS.PHONE && (
            <>
              ...
              <p className="text-ink-500 text-sm mb-6">We'll text you a one-time code to confirm it's really you.</p>
              <form onSubmit={handleSendOtp} className="space-y-4">
                <Input label="Owner Phone Number" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91XXXXXXXXXX" />
                <p className="text-[11px] text-ink-500">Use an international format like +91XXXXXXXXXX for the OTP test.</p>
          ...
          {step === STEPS.OTP && (
            <>
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                <Input label="6-digit OTP" required value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="123456" maxLength={6} />
                {error && <p className="text-xs text-vitality-critical">{error}</p>}
          ...
          {step === STEPS.DETAILS && (
            <>
              ...
              <p className="text-ink-500 text-sm mb-6">Phone number verified ✓. Just a few more details.</p>
              <form onSubmit={handleCreateGym} className="space-y-4">
                <Input label="Gym Name" required value={form.gymName} onChange={handleField('gymName')} />
                <Input label="Owner Name" required value={form.ownerName} onChange={handleField('ownerName')} />
                <Input label="Email (optional)" type="email" value={form.email} onChange={handleField('email')} />
                <Input label="Password" type="password" required value={form.password} onChange={handleField('password')} />
                <div className="grid grid-cols-2 gap-3">
                  <Input label="City" required value={form.city} onChange={handleField('city')} />
                  <Input label="State" required value={form.state} onChange={handleField('state')} />
                </div>
                <Input label="Short Address" required value={form.shortAddress} onChange={handleField('shortAddress')} />
                {error && <p className="text-xs text-vitality-critical">{error}</p>}
                <Button type="submit" loading={loading} className="w-full">Create Gym Account</Button>
              </form>
            </>
          )}
        </Card>
